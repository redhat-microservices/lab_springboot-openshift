...
[source]
----
forge -e "project-add-repository --url http://repo.spring.io/libs-snapshot-local --named spring-cloud-snapshot"
...
. Add the role `view` to the user `serviceaccount` to let the Spring Cloud Kubernetes API to access the ConfigMap
...
[source]
----
forge -e "fabric8-setup --integration-test=false"
----
