...
. One of the goal of this workshop is to show you how easy it could be with Spring to use JavaEE standards as JAX-RS. For that purpose we will add the Apache CXF Spring Boot
  starter which provides such implemetnation like also a JSon PArser framework to generate the JSon responses
+
[source]
----
project-add-dependencies org.apache.cxf:cxf-spring-boot-starter-jaxrs:3.1.11
project-add-dependencies com.fasterxml.jackson.jaxrs:jackson-jaxrs-json-provider:2.8.8
----
...
. Add some config to let Apache CXF to discover the REST classes to be registered like also the path to map with its CXF Servlet.
  Edit the application.properties file and add these keys
+
[source]
----
...
. Add a JSon Provider bean within the `DemoApplication` java class who will populate the HTTP Body response
+
[source]
----
...
. The last step of this module section will consist to use a JBoss Forge scaffold command to populate the static Web content.
  The AngularJS application created contains the screens allowing to perform the CRUD operations
  against the Catalog REST service exposed by this endpoint `http://localhost:8080/rest/catalogs`.
  The content is generated using the `CatalogEndpoint` as input created previously within the `cdservice` project.
...
. OR copy content from `scripts/front/modified` directory
+
[source]
----
...
. Verify first that you are well connected to the `OpenShift` platform
...
To externalize the datasource configuration that SpringBoot will use to access the database, we will create a Kubernetes `configMap` resource.
This `ConfigMap` will contain as input the content of the application.properties file. The resource to be created will be defined under a json `configmap.json` file
under tje folder `src/main/fabric8`. All the resources included within this folder, will be scanned and used by the Fabric8 Maven Plugin when the application will be deployed on Openshift.
...
. In order to expose our docker container as a Service, we will create a `service` resource. The content of the file `svc.yml` will be used
  by Kubernetes to expose using its Api Gateway a service available externally under the specified port. The targetPort allows to map the docker port with the targetPort
  exposed by the Api.
...
The URL to access the service will be specified within the `service.json` file that the AngularJS framework will load when the service `/catalogs`
will be called.

NOTE: The file isn't mounted as a volume attached to the pod but that could be done using the same mechanism as presented before.
...
WARNING: To be done when code has been generated
