...
WARNING: If you deploy your poeject on GCP, it is not possible to use persistent volume and then you must install the ephemeral mysql DB
using this command `oc create -f https://raw.githubusercontent.com/openshift/origin/v1.5.1/examples/db-templates/mysql-ephemeral-template.json`
...
. Disable DB Health Check. So edit the ConfigMap and this line
+
----
...
. Open the Hystrix dashboard and add the ADDRESS of the Cd service `http://cdservice-workshop.192.168.64.25.nip.io/hystrix.stream`
  to view the status of the circuit breaker. It should be `close`
+
