...
The OpenShift client tool is packaged within the Minishift distribution (since version 1.0). So, use the command `minishift oc-env` to display the command you need to type into your shell in order to add the `oc` binary to your `PATH`. The output of oc-env will differ depending on OS and shell type.
