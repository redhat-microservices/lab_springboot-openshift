...
forge -i org.jboss.forge.addon:spring-boot,1.0.0-SNAPSHOT
----
...
NOTE: To use the scaffold option, pass the boolean true to the command ` ./scripts/setup.sh demo true`
...
To externalize the datasource configuration that Spring Boot will use to access the database from the `cdservice` project, we will create a Kubernetes `configMap` resource.
...
. Add dependency to the project to use Spring Cloud Kubernetes Core lib which allows Spring Boot to access the config map and read the application properties
  property
+
[source]
----
...
. Add bootstrap.properties to specify the name of the application to be used and which corresponds to the key of the ConfigMap to search
+
[source]
----
touch src/main/config-openshift/bootsrap.properties

cat << 'EOF' > src/main/config-openshift/bootsrap.properties
spring.application.name=cdservice
----

. Add the role `view` to the user `serviceaccount` to let the Spring Cloud Kubernetes API to access the ConfigMap exposed by the Kuberntes Api
+
[source]
----
