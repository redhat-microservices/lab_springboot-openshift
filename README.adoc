...
Within this section, we will implement the circuit breaker pattern using the https://github.com/Netflix/Hystrix[NetFlix OSS Hystrix] project. The breaker will be developed within
...
The command contains 2 methods `run()` and `fallback()` which are used by the `HystrixServlet` with the help of the Java observable pattern. The method run will be called regularly to check if we get a response from the MySQL database,
if this is the case, the fallback method will be called. The information (= events or hearbeat messages) created, are published by Hystrix regularly on the
