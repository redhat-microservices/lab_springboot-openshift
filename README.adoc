...
. Add the role `view` to the default `serviceaccount` to let the Spring Cloud Kubernetes API to access the ConfigMap exposed by the Kubernetes Api. This serviceaccount
is the pod's user that the application will use to access the Kubernetes backend.
+
[source]
----
