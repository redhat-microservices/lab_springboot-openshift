...
. To be able to use the project locally but also on OpenShift, we will define 2 datasources and JDBC drivers to use either a H2 in-memory
  database which doesn't required any installation of a database or MySQL that we will install on OpenShift.
...
. Generate the static content using the REST endpoints defined within the cdservice project
  The last step of this module section will consist to use a scaffold command to populate the static Web content.
  This AngularJS application contains the screens required to perform the CRUD operations to call the REST services `http://localhost:8080/rest/catalogs`

. First create the missing folder where the static content will be generated
+
[source]
----
mkdir -p cdfront/src/main/resources/static
----

. Next, scaffold the code within the `cdservice` project
+
[source]
----
